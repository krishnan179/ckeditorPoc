...
import ClassicEditorBuild from 'ckeditor5-custom-build/build/ckeditor';
...
interface LoadSaveIntegrationState {
	isLayoutReady: boolean;
	editor: any;
	licenseKey: string;
}

export default class LoadSaveIntegration extends Component<{}, LoadSaveIntegrationState> {
	private sidebarElementRef: any;
	private editorContainerRef: any;
	private viewerContainerElementRef: any;
	private viewerEditorElementRef: any;
	private viewerSidebarContainerElementRef: any;
	...
	componentDidMount() {
		...
		const licenseKey: any = window.localStorage.getItem( STORAGE_KEY ) || window.prompt( 'Your license key' );
		this.setState( {
								...
								config={ {
									...
									autosave: {
										save: async (editor:any) => {
											...
											const saveData = async ( documentData:any, revisionData:any ) => {
		...
		return (
			<footer>
				<div className="centered">
					...
					<p>
						Copyright © 2003-2023, <a href="https://cksource.com/" target="_blank" rel="noopener noreferrer">CKSource Holding sp. z o.o.</a> – All rights reserved.
					</p>
				</div>
			</footer>
		);
	...
	showEditorDataInConsole( evt:any ) {
	...
	checkPendingActions( domEvt:any ) {
		if ( !this.state.editor ) {
			return;
		}
...
class CommentsIntegration {
	...
	init() {
		const commentsRepositoryPlugin:any = this.editor.plugins.get( 'CommentsRepository' );

		// // Load the comment threads data.
		// if(appData.commentThreads.length>0){
		// 	for ( const commentThread of appData.commentThreads ) {
		// 		if(commentThread?.isFromAdapter){
